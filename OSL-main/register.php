...
include('database.php');
  $username = $_POST['username'];
	...
	$password = md5($pass);
	$email = $_POST['email'];
if(isset($_POST['signupplr']))
{
$sql="INSERT INTO  player_reg_tbl (p_username, p_email, p_password) VALUES('$username','$email','$password')";
...
else if(isset($_POST['signupcoach']))
	{
	$sql="INSERT INTO  coach_reg_tbl (c_username, c_email, c_password) VALUES ('$username','$email','$password')";
  $query = mysqli_query($con,$sql);
  if ($query)
  {
	echo "<script>alert('Registration successfull. Now you can login');</script>";
...
else
{
	...
	{
	$sql="INSERT INTO  club_reg_tbl (cl_username, cl_email, cl_password) VALUES ('$username','$email','$password')";
  $query = mysqli_query($con,$sql);
  if ($query)
  {
	echo "<script>alert('Registration successfull. Now you can login');</script>";
...
<div class="signup-form">
    <form  method="post" name ='signup' onSubmit="return valid();">
		...
		<div class="form-group">
		       <center> <button type="submit" id='submitplr' name='signupplr' class="btn btn-primary btn-lg">Player</button>
		  <button type="submit" id='submitcoach' name='signupcoach' class="btn btn-primary btn-lg">Coach </button>
      <button type="submit" id='submitclub' name='signupclub' class="btn btn-primary btn-lg">Club</button></center>
		</div>
    </form>
