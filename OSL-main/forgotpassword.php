...
{
	$sql = "select * from player_reg_tbl where p_email='$email'";
	$query = mysqli_query($con,$sql);
	$row = mysqli_num_rows($query);
	if ($row>0)
		{
			$sql1="update player_reg_tbl set p_password='$password' where p_email='$email'";
		...
		{
			$sql = "select * from coach_reg_tbl where c_email='$email'";
			$query = mysqli_query($con,$sql);
			$row = mysqli_num_rows($query);
			if ($row>0)
				{
					$sql1="update coach_reg_tbl set c_password='$password' where c_email='$email'";
					$query1 = mysqli_query($con,$sql1);
					echo "<script>alert('Updation successfull. Now you can login');</script>";
					echo "<script type=text/javascript>window.location.href = index.php</script>";
				}
				...
				{
					$sql = "select * from club_reg_tbl where cl_email='$email'";
					$query = mysqli_query($con,$sql);
					$row = mysqli_num_rows($query);
					if ($row>0)
						{
							$sql1="update club_reg_tbl set cl_password='$password' where cl_email='$email'";
							$query1 = mysqli_query($con,$sql1);
							echo "<script>alert('Updation successfull. Now you can login');</script>";
							echo "<script type=text/javascript>window.location.href = index.php</script>";
			    	}
